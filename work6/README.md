...
## 六、实验现象与结果分析（可据你本地日志补充数据）

1. **初期**：光源位于球体偏背面时，在 Leaky 模型下仍有非零梯度，`loss` 可缓慢下降，光源位置沿可微方向“绕到”受光侧；当光越过临界方位后，**MSE 常出现更快下降**（与讲义描述的“断崖式”现象一致，具体数值与步长、随机性有关）。  
...
（此处可粘贴你最近一次运行的终端：`loss`、**light**、**albedo**、**shininess** 末行数值。）

---

## 七、问题与总结

1. Taichi **可微程序**需注意并行核与 `Tape` 搭配时的结构限制（如损失与像素循环的组织方式）。  
